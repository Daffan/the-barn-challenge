...
class HybridEncoder(Encoder):
    def __init__(self,
        ...
        self.net = nn.LSTM(input_size=260,
                          hidden_size=hidden_size,
                          num_layers=num_layers,
                          batch_first=True)
    ...
    def forward(self, x):
        ...
        """
        # h0 = torch.zeros(self.num_layers, x.shape[0], self.hidden_size).to(x.device)
        l = self.embedding(x[..., :720])
        f = x[..., 720:]
        x = torch.cat([l, f], dim=-1)
        output, *_ = self.net(x)
        y = output[:,-1]
        return y

'''
class HybridTransformerEncoder(Encoder):
    def __init__(self,
                 input_dim,
                 num_layers=2,
                 hidden_size=512,
        ...
        self.embed_state = torch.nn.Linear(260, hidden_size)
        self.embed_ln = nn.LayerNorm(hidden_size)

    def forward(self, states):
        l = self.embedding(states[..., :720])
        f = states[..., 720:]
        states = torch.cat([l, f], dim=-1)
